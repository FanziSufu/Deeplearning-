...
print('Training set:', train_dataset.shape, train_labels.shape)
print('Validaion set:', valid_dataset.shape, valid_labels.shape)
print('Test set:', test_dataset.shape, test_labels.shape)
...
with graph.as_default():
    ...
    layer3_weights = tf.Variable(tf.truncated_normal(shape=(image_size // 4 * image_size // 4 * depth, num_hidden), stddev=0.1))
...
with graph.as_default():
    ...
    def model(dataset):
        conv1 = tf.nn.conv2d(dataset, layer1_weights, [1, 1, 1, 1], padding='SAME')
        ...
        reshape = tf.reshape(pool2, shape=(shape[0], shape[1] * shape[2] * shape[3]))
...
with graph.as_default():
    ...
    def model(dataset, keep_prob):
        ...
        reshape = tf.reshape(pool2, shape=(shape[0], shape[1] * shape[2] * shape[3]))
        hidden3 = tf.nn.relu(tf.matmul(reshape, layer3_weights) + layer3_biases)
        drop3 = tf.nn.dropout(hidden3, keep_prob)
    ...
    train_predictions = tf.nn.softmax(logits)
    valid_predictions = tf.nn.softmax(model(tf_valid_dataset, 1.0))
    test_predictions = tf.nn.softmax(model(tf_test_dataset, 1.0))
